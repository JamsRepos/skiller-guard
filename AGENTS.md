...
- Config group names must be specific — e.g. `"skiller-guard"`, not `"skiller"`.
...
- Rename everything from the template. Do not leave `com.example`, `ExamplePlugin`, `ExampleConfig`, or `example` as the config group.
