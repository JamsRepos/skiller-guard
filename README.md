...
| --- | --- | --- |
| Block Prayer XP | On | Removes **Bury** / **Scatter** on every bone and ashes (including GOTR reward ashes), offering at named altars/shrines, Take on ground remains, Pyramid Plunder **Open Sarcophagus** |
| Block combat training | On | Dummy Attack/Hit (including combat/slayer dummies), firing a dwarf cannon, and **Cast on the spellbook** (Home Teleport stays; it gives no Magic XP) |
...
Edit `SafetyCatalog` with a specific option/target glob. Prefer option names (`Bury`, `Scatter`) over `*bone*` wildcards — those hide bone crossbows. NPC hides that key off NPC type must stay in the opt-in NPC pack.

Named overhead NPCs go in `NamedNpcCatalog` (exact names). Combat-XP quest titles go in `QuestDenylist`.
